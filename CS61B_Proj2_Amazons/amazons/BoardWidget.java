...
class BoardWidget extends Pad {
    ...
    static final Color
        ...
        CLICK_COLOR = new Color(142, 205, 249);
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int x = 0; x < 10; x += 2) {
            for (int y = 0; y < 10; y += 2) {
                ...
                g.fillRect(cx(x), cy(y), SQUARE_SIDE, SQUARE_SIDE);
            }
        }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        while (sqs.hasNext()) {
            Square next = sqs.next();
            if (_board.get(next) == BLACK) {
                drawQueen(g, next, BLACK);
            } else if (_board.get(next) == WHITE) {
                drawQueen(g, next, WHITE);
            } else if (_board.get(next) == SPEAR) {
                g.setColor(SPEAR_COLOR);
                g.fillRect(cx(next.col()), cy(next.row()),
                        SQUARE_SIDE, SQUARE_SIDE);
            }
        }
        if (_clicks[0] != null) {
            g.setColor(CLICK_COLOR);
            g.fillRect(cx(_clicks[0]), cy(_clicks[0]),
                    SQUARE_SIDE, SQUARE_SIDE);
        }
        if (_clicks[1] != null) {
            g.setColor(CLICK_COLOR);
            g.fillRect(cx(_clicks[1]), cy(_clicks[1]),
                    SQUARE_SIDE, SQUARE_SIDE);
        }
    }
    ...
    private void click(Square s) {
        if (_clicks[0] == null && _board.isLegal(s)) {
            _clicks[0] = s;
        } else if (_clicks[1] == null && _board.isLegal(_clicks[0], s)) {
            _clicks[1] = s;
        } else if (_board.isLegal(_clicks[0], _clicks[1], s)) {
            String result = _clicks[0].toString() + " "
                    + _clicks[1].toString() + " "
                    + s.toString();
            _clicks = new Square[2];
            _commands.offer(result);
        }
        repaint();
    }
    ...
    /** Array that records clicks.*/
    private Square[] _clicks = new Square[2];
}
